...
/// Ludo v2 — board painter.
///
/// Renders the static Ludo board (four home quadrants, the centre
/// destination triangles, the cross track with safe spots) and computes the
/// per-player track rectangles, handed back to the caller via the
/// [trackCalculationListener] callback. Rendering logic is preserved verbatim
/// from fludo; only theming/style touch-ups were applied.
///
/// Board layout:
/// ```
///  _________
...
class BoardPainter extends CustomPainter {
  ...
  final void Function(List<List<List<Rect>>> tracks) trackCalculationListener;
  ...
  void _calculatePlayerTracks() {
    ...
    // Player 1 track
    final List<Rect> playerOneTrack = [];
    for (int stepIndex = 0; stepIndex < 57; stepIndex++) {
      if (stepIndex == 0) {
        final offset = _stepSize / 2;
    ...
    // Player 2 track
    final List<Rect> playerTwoTrack = [];
    ...
    // Player 3 track
    final List<Rect> playerThreeTrack = [];
    ...
    // Player 4 track
    final List<Rect> playerFourTrack = [];
    ...
    // Combine spawn spots with their tracks per player.
    final List<List<List<Rect>>> playerTracks = [];
    for (int playerIndex = 0; playerIndex < 4; playerIndex++) {
      final List<List<Rect>> playerTrack = [];

      for (int spotIndex = 0;
          spotIndex < _homeSpotsList[playerIndex].length;
          spotIndex++) {
        final List<Rect> track = [];
  ...
  void _drawHome(Canvas canvas, Size size) {
    // Home bases (one per player quadrant).
    _fillPaint.color = LudoV2Colors.home1;
    final home1 = Rect.fromLTWH(0, 0, _homeSize, _homeSize);
    canvas.drawRect(home1, _fillPaint);
    canvas.drawRect(home1, _strokePaint);

    _fillPaint.color = LudoV2Colors.home2;
    final home2 = Rect.fromLTWH(_homeStartOffset, 0, _homeSize, _homeSize);
    canvas.drawRect(home2, _fillPaint);
    canvas.drawRect(home2, _strokePaint);

    _fillPaint.color = LudoV2Colors.home3;
    final home3 = Rect.fromLTWH(
        _homeStartOffset, _homeStartOffset, _homeSize, _homeSize);
    canvas.drawRect(home3, _fillPaint);
    canvas.drawRect(home3, _strokePaint);

    _fillPaint.color = LudoV2Colors.home4;
    final home4 = Rect.fromLTWH(0, _homeStartOffset, _homeSize, _homeSize);
    canvas.drawRect(home4, _fillPaint);
    canvas.drawRect(home4, _strokePaint);

    // Inner home (white interior).
    final innerHomeSize = _homeSize - 2 * _stepSize;

    _fillPaint.color = Colors.white;
    final innerHome1 = Rect.fromLTWH(home1.left + _stepSize,
    ...
    final innerHome2 = Rect.fromLTWH(home2.left + _stepSize,
    ...
    final innerHome3 = Rect.fromLTWH(home3.left + _stepSize,
    ...
    final innerHome4 = Rect.fromLTWH(home4.left + _stepSize,
    ...
    // Spawn spots.
    _drawSpawnSpots(canvas, innerHome1, LudoV2Colors.home1);
    _drawSpawnSpots(canvas, innerHome2, LudoV2Colors.home2);
    _drawSpawnSpots(canvas, innerHome3, LudoV2Colors.home3);
    _drawSpawnSpots(canvas, innerHome4, LudoV2Colors.home4);
  }

  void _drawSpawnSpots(Canvas canvas, Rect innerHome, Color color) {
    final List<Offset> spotList = [];

    _fillPaint.color = color;
    final spotOffsetOne = innerHome.width / 4;
    final spotOffsetTwo = 3 * spotOffsetOne;
    final double spotRadius = spotOffsetOne / 2;

    canvas.save();
    canvas.translate(innerHome.left, innerHome.top);

    final spot1 = Offset(spotOffsetOne, spotOffsetOne);
    canvas.drawCircle(spot1, spotRadius, _fillPaint);
    canvas.drawCircle(spot1, spotRadius, _strokePaint);

    final spot2 = Offset(spotOffsetTwo, spotOffsetOne);
    canvas.drawCircle(spot2, spotRadius, _fillPaint);
    canvas.drawCircle(spot2, spotRadius, _strokePaint);

    final spot3 = Offset(spotOffsetOne, spotOffsetTwo);
    canvas.drawCircle(spot3, spotRadius, _fillPaint);
    canvas.drawCircle(spot3, spotRadius, _strokePaint);

    final spot4 = Offset(spotOffsetTwo, spotOffsetTwo);
    canvas.drawCircle(spot4, spotRadius, _fillPaint);
    canvas.drawCircle(spot4, spotRadius, _strokePaint);

    canvas.restore();

    // Cache spot coordinates so tracks can begin from the correct home spawn.
    final left = innerHome.left + spotOffsetOne;
    final right = innerHome.left + spotOffsetTwo;
    final up = innerHome.top + spotOffsetOne;
    final down = innerHome.top + spotOffsetTwo;
    ...
    for (int homeIndex = 0; homeIndex < 4; homeIndex++) {
      ...
      for (int pos = 0; pos < 6; pos++) {
        ...
        if (pos > 0) {
          ...
          final arrowPadding = unit.width / 4;
          final arrowWingGap = arrowPadding / 1.5;
          final arrowTip = Offset(
              unit.right - arrowPadding, unit.bottom - unit.height / 2);
      ...
      for (int pos = 0; pos < 6; pos++) {
        ...
        if (pos == 2) {
          final safeSpotRadius = _stepSize / 4;
          _fillPaint.color = LudoV2Colors.safeSpot;
